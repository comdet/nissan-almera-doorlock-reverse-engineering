...
**หมายเหตุ:**
- D กับ L ค่าเดียวกัน (0xC0) แยกไม่ได้จาก byte นี้อย่างเดียว
- Byte 27 เป็น composite — ตอนเครื่องดับค่าผสม handbrake/engine state เข้ามา
- **ค่าข้างบนเชื่อถือได้เฉพาะตอนเครื่องติด** ตอนเครื่องดับใช้ RPM=0 แทน
...
### Byte 3: ไม่ใช่ Engine On/Off Indicator

| ค่า | สถานะ |
|---|---|
| 0x10 | ACC/ON mode + เกียร์ P (ไม่ว่าเครื่องติดหรือดับ) |
| 0x40 | เกียร์ N (เครื่องติด) |

**ไม่เปลี่ยนตาม engine on/off** — เปลี่ยนตามเกียร์แทน ไม่ใช้สำหรับ detect engine
...
- [x] Gear position (P/R/N/D/L) — DID 0x0108 byte 27 ✅ (engine ON only)
