...
static void updateStateMachine() {
    ...
    case DrvState::ENGINE_OFF:
        // Countdown before firing auto-unlock. unlock_delay is NVS-configurable
        // (default 1s — tuned down from the original 3s after the stateless
        // engine-off detection landed). By the time we're in this state we've
        // already confirmed RPM=0 plus either gear=P or 2s of ECU silence,
        // so the "false engine off" risk is low. Keep > 0 so a brief stall +
        // restart can still cancel via the engine_running check below.
        if (engine_running) {
